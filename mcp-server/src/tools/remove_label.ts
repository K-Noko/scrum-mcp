...
export async function removeLabel(args: z.infer<typeof removeLabelSchema>) {
  ...
  const updated = current.filter((l) => l !== args.label);

  // ARRAY_AGG で既存ラベルから除外して再構築
  await query(
    `UPDATE \`${dataset}.tasks\`
     SET labels = (
       SELECT ARRAY_AGG(l) FROM UNNEST(labels) l WHERE l != ?
     ),
     updated_at = ?
     WHERE task_id = ?`,
  ...
  await query(
    `INSERT INTO \`${dataset}.task_history\`
     ...
     VALUES (?, ?, ?, 'labels', ?, ?, ?)`,
    [`hist_${nanoid(10)}`, args.task_id, userId, JSON.stringify(current), JSON.stringify(updated), now]
  );

  return { task_id: args.task_id, labels: updated, changed: true };
}
